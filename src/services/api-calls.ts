...
export async function searchMovieCriteria(criteria: string) {
  let rawData = await fetch(
...
export async function fetchMoviesById(ids: number[]) {
  let movieData: Movie[] = [];
  ids.forEach(async (id: number) =>{
  let rawData = await fetch(
    `${root2}movie/${id}?api_key=${movie_api_key}&language=en-US`
  )
  movieData += await rawData.json();
})
